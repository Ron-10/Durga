...
import { Calendar, Clock, MapPin, Users, Image, Sparkles, Search, Filter, ChevronRight, Star, Heart, Share2, Eye, Frown } from 'lucide-react';
...
const EventsPage = () => {
  ...
  // --- NEW LOGIC TO CONTROL VISIBILITY ---
  // The featured event section will only show if 'All' is selected AND the search bar is empty.
  const showFeatured = selectedCategory === 'All' && searchTerm === '';

  // --- DYNAMIC TITLE FOR THE EVENTS LIST ---
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="backdrop-blur-lg bg-white/20 rounded-3xl shadow-2xl border border-white/30 p-6 md:p-8">
            <div className="relative max-w-md mx-auto mb-6">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-white/50 backdrop-blur-sm border border-white/30 rounded-2xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all duration-300"
              />
            </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              {categories.map((category) => (
                <motion.button
                  ...
                  className={`px-5 py-2 rounded-xl font-semibold transition-all duration-300 text-sm md:text-base ${
        ...
        {/* --- CONDITIONALLY RENDERED FEATURED EVENT --- */}
        <AnimatePresence>
              ...
              {filteredEvents.length > 0 ? (
                filteredEvents.map((event) => (
                  <motion.div
                      ...
                      className="flex flex-col backdrop-blur-lg bg-white/20 rounded-3xl shadow-2xl border border-white/30 overflow-hidden hover:bg-white/30 transition-all duration-300 cursor-pointer"
      ...
      {/* --- Event Modal (No Changes Needed Here) --- */}
      <AnimatePresence>
        {selectedEvent && (
            <motion.div
